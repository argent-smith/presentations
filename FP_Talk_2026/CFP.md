...
Доклад раскрывает фундаментальные свойства ФП через призму пяти языков с разной философией: OCaml (академическая чистота), Scala (JVM-прагматизм), Python (простота), JavaScript (вездесущесть) и Rust (системная безопасность).
...
### 2. Чистота функций: почему ваш код непредсказуем
...
4. **JavaScript** — вездесущесть и функции первого класса
...
| Чистота функций        | Команды без глобального состояния         |
